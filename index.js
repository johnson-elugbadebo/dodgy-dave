  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        // const url = `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/day/${dates.startDate}/${dates.endDate}?adjusted=true&sort=asc&apiKey=${import.meta.env.VITE_POLYGON_API_KEY}`;
        // console.log('Fetching URL (without API key):', url.split('apiKey=')[0]);

        // https://polygon-api-worker.johnson-elugbadebo.workers.dev/?ticker=GOOG&startDate=2025-03-21&endDate=2025-03-23

        const url = `https://polygon-api-worker.johnson-elugbadebo.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;

        const response = await fetch(url);
        // console.log(`Response status for ${ticker}:`, response.status);
        // console.log(`Response status text for ${ticker}:`, response.statusText);

        if (!response.ok) {
          const errorText = await response.text();
          console.log(`Error response for ${ticker}:`, errorText);
...
async function fetchReport(data) {
  ...
  try {
    ...
    // Fetch request to CloudFlare worker
    const url = 'https://openai-api-worker.johnson-elugbadebo.workers.dev/';
    const response = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    loadingArea.innerText = 'Unable to access AI. Please refersh and try again';
  }
}
